...
import { Delete, Get, Patch, Post, PostFormData, Put, PutFormData } from './http.type'

// An singleton axio instance to fetch the api calls

export class HttpInstance {
  private static instance: HttpInstance
  ...
  private axioInstance: AxiosInstance = axios.create({
    ...
    headers: {
      ...
      Authorization: this.appAuth.getUserToken() ? `Bearer ${this.appAuth.getUserToken()}` : ''
    }
  })
